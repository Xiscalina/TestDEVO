...
//Normalizamos formato (categoria en mayúsculas, fecha en ms y valor)
...
ejercicio2.prototype.cargarSeriesGraficos = function () {
    ...
    for(var i=0; i<unicCateg.length; i++){
      ...
      for (var j=0; j<serieCateg.length; j++){
        var index=this.indiceArray(serieCateg[j].fecha,his.seriesLineas[i].datos)
        if (index==-1){this.seriesLineas[i].datos.push([serieCateg[j].fecha,serieCateg[j].valor]);
        else {this.seriesLineas[i].datos[index][1] += serieCateg[j].valor)} 
        categ.total += serieCateg[j].valor;
      }
      resumenCateg.categ.push(categ);
      resumenCateg.total += categ.total;
